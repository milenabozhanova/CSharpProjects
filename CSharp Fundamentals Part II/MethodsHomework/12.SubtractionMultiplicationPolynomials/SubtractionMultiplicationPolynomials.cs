...
class SubtractionMultiplicationPolynomials
{
    static void Main()
    {
        ...
        int[] resSum = SubtractPloynomials(func1, func2, max);
        Console.WriteLine("The coefficients: {0}", string.Join(" , ", resSum));
        Console.WriteLine();

        int[] resMult = CalcMultiplication(func1, func2, max1 + max2 + 1);
        Console.WriteLine("Multiplication:");
        Console.WriteLine("The coefficients: {0}", string.Join(" , ", resMult));
    }

    static int[] CalcMultiplication(SortedList<int, int> func1, SortedList<int, int> func2, int max)
    {
        int[] res = new int[max];

        SortedList<int, int> mult = new SortedList<int, int>();
        foreach (KeyValuePair<int, int> it1 in func1)
        {
            foreach (KeyValuePair<int, int> it2 in func2)
            {
                AddValue(mult, it1.Key + it2.Key, it1.Value * it2.Value);
            }
        }

        for (int i = 0; i < max; i++)
        {
            if (mult.Keys.Contains(i))
            {
                res[i] += mult[i];
            }
        }
    ...
    static int[] SubtractPloynomials(SortedList<int, int> func1, SortedList<int, int> func2, int max)
    {
        int[] res = new int[max];
